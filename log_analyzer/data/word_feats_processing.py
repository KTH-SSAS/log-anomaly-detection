...
Edited for correctness (redteam flag setting) by Simon Gokstorp.
"""
# NOTE: seems to only work with Python2
# Example usage: python2 word_feats_processing.py -datafile=auth.txt -outfile=word_feats.txt -record_dir=temp -redfile=redteam.txt
...
def return_parser():
    ...
    parser = argparse.ArgumentParser(
        "Convert raw loglines to ASCII code (minus 30) integer representation."
    )
    parser.add_argument(
        "-datafile",
        type=str,
    ...
    parser.add_argument("-outfile", type=str, help="Where to write derived features.")
    parser.add_argument(
        "-record_dir",
        type=str,
...
def translate_line(string):
    ...
    time = int(
        data[0]
    )  # could be used to make categorical variables for day of week and time of day.
...
if __name__ == "__main__":
    ...
    with open(args.datafile, "r") as infile, open(args.outfile, "w") as outfile:
        ...
        for line_num, line in enumerate(infile):
            ...
            # print(f"line: {line}")
            # red_line = redevents.pop()
            # redevents.add(red_line)
            # print(f"line in redteam style: {red_style_line}")
            # print(f"example redevent line: {red_line}")
            if user.startswith("U") and day not in weekend_days:
                outfile.write(
                    "%s %s %s %s %s %s"
                    % (
